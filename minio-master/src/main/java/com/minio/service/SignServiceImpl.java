...
public class SignServiceImpl implements SignService {
    ...
    public TokenDTO getSign(String appId) throws UnsupportedEncodingException {
        ...
        String secret = BCryptUtil.hashpw(sign.getAppId(), sign.getSalt());

        TokenDTO tokenDTO = new TokenDTO();
        tokenDTO.setAppId(appId);
        tokenDTO.setAccessKey(encoder.encodeToString(access.getBytes(StandardCharsets.UTF_8)));
        tokenDTO.setSecretKey(encoder.encodeToString(secret.getBytes(StandardCharsets.UTF_8)));
